  ...
  <table class="table ssis-data-table w-full">
    <thead>
      <tr class="ssis-thead border-b border-(--border)">
        <SortHeader label="ID" sortBy={sortBy} order={order} sortKey="s.id" onSort={onSort} />
        <SortHeader label="First Name" sortBy={sortBy} order={order} sortKey="s.first_name" onSort={onSort} />
        <SortHeader label="Last Name" sortBy={sortBy} order={order} sortKey="s.last_name" onSort={onSort} />
        <SortHeader label="Program" sortBy={sortBy} order={order} sortKey="p.code" onSort={onSort} />
        <SortHeader label="Year" sortBy={sortBy} order={order} sortKey="s.year" onSort={onSort} />
        <SortHeader label="Gender" sortBy={sortBy} order={order} sortKey="s.gender" onSort={onSort} />
        <ActionsHeader />
      </tr>
    </thead>

    <tbody class="divide-y divide-(--border-subtle)">
      <TableShell loading={loading} hasData={rows.length > 0} colspan={7} {emptyMessage}>
        {#each rows as student}
          <tr class="ssis-row transition-colors hover:bg-(--bg-hover)">
            ...
            <td class="ssis-cell-muted px-4 py-3.5">
              <span class="ssis-code-pill">{student.program?.code || 'NULL'}</span>
            </td>
